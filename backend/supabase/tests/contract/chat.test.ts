...
Deno.test("chat: a BYOK request bypasses the per-user quota (real key-rejection round trip)", async () => {
  ...
  const { error: seedError } = await admin
    .from("ai_usage")
    .upsert({ user_id: userA.id, day, endpoint: "chat", count: 100000 }, { onConflict: "user_id,day,endpoint" });
  ...
  try {
    ...
    assertEquals(res.status, 402, `expected 402 (claude_key_rejected), got ${res.status}: ${res.text.slice(0, 200)}`);
    ...
    const { data: usage } = await admin
      ...
      .eq("day", day)
      .single();
    assertEquals(usage?.count, 100000, "a BYOK request must not touch the per-user quota counter");
    ...
    const { data: byokUsage } = await admin
      ...
      .eq("day", day)
      .single();
    assertEquals(byokUsage?.count, 1, "a BYOK request should still be recorded for observability");
  } finally {
