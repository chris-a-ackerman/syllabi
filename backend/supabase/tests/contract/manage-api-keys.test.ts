...
Deno.test(
  'save-anthropic-key: an Anthropic-rejected key gets 400, stores nothing, and never echoes the key',
  async () => {
    const { userA } = await getFixtures();
    try {
      const res = await callFn('save-anthropic-key', {
        token: userA.token,
        body: { key: GARBAGE_KEY },
      });
...
Deno.test(
  'test-anthropic-key: a stored key the real API rejects flips last_test_ok false, never echoes the key',
  async () => {
    const { userA } = await getFixtures();
...
Deno.test(
  'manage-api-keys: 429 once the shared daily limit is spent (test-anthropic-key)',
  async () => {
    const { userA } = await getFixtures();
...
Deno.test(
  'manage-api-keys: 429 once the shared daily limit is spent (save-anthropic-key)',
  async () => {
    const { userA } = await getFixtures();
    await seedManageApiKeysUsageAtLimit(userA.id);
    try {
      ...
      assertEquals(res.status, 429, `expected 429, got ${res.status}: ${res.text.slice(0, 200)}`);
      assertEquals(
        await hasAnthropicKey(userA.token),
        false,
        'a rate-limited save must store nothing'
      );
    } finally {
      await clearManageApiKeysUsage(userA.id);
      await clearAnthropicKey(userA.id);
