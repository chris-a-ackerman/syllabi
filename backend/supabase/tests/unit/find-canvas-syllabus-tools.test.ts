...
Deno.test(
  'get_course_modules never fetches an off-host or blocked-range item.url; those items come back unavailable',
  async () => {
    ...
    try {
      const results = await executeTools([modulesToolUse()], 'test-token', BASE_URL, ALLOWED_HOST);
...
Deno.test(
  'get_course_modules skips a File item whose url is not a string, without fetching it',
  async () => {
    const modulesPayload = [
      {
        id: 1,
        name: 'Week 1',
        items: [{ title: 'Weird item', type: 'File', url: 12345 }],
      },
    ];
