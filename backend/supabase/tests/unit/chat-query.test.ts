...
Deno.test("detectQueryType: 'ai' matches only as a standalone word", () => {
  assertEquals(
    detectQueryType(
      "Can you send the professor's contact via mail merge? Also his mailing address"
    ),
    'general'
  );
...
Deno.test(
  "extractDateRange: multi-word phrases win over 'today' appearing in the same message",
  () => {
...
Deno.test('buildCourseContext: null meeting_times start/end render TBD, not an empty dash', () => {
  const course = {
    ...COURSE,
    schedule: { ...COURSE.schedule, meeting_times: { start: null, end: null } },
  };
  const out = buildCourseContext([course], [], 'general');
...
Deno.test(
  'buildCourseContext: a null or non-finite weight renders as unknown, not null%/NaN%',
  () => {
    const course = {
      ...COURSE,
      grading_rules: {
...
Deno.test(
  'buildCourseContext: only a start time renders the start alone, not a trailing dash',
  () => {
    const course = {
      ...COURSE,
      schedule: { ...COURSE.schedule, meeting_times: { start: '10:00', end: null } },
    };
    const out = buildCourseContext([course], [], 'general');
...
Deno.test('buildCourseContext: only an end time renders the end alone, not a leading dash', () => {
  const course = {
    ...COURSE,
    schedule: { ...COURSE.schedule, meeting_times: { start: null, end: '11:15' } },
  };
  const out = buildCourseContext([course], [], 'general');
