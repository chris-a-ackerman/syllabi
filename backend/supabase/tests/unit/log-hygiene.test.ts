...
function isUnsafeReference(braceContent: string, key: string): boolean {
  ...
  return !(safeBoolean.test(braceContent) || safeLength.test(braceContent) || safeRedact.test(braceContent));
}
...
Deno.test("no console.* call in a Canvas function logs a raw URL, payload dump, or verifier token", async () => {
  const violations: string[] = [];
...
Deno.test("log-hygiene checker: allows plain label text mentioning the field name outside braces", () => {
