          ...
          return {
            in(column: string, ids: string[]) {
              ...
              if (isErrorChunk) {
                return Promise.resolve({
                  data: null,
                  error: options.deleteError ?? 'delete failed',
                });
              }
...
Deno.test(
  'insert error → ok:false, stage:insert, and no delete call is issued (old rows untouched)',
  async () => {
...
Deno.test(
  'success with 250 pre-existing ids → one insert, three delete calls partitioning all 250 ids',
  async () => {
    ...
    const result = await replaceCourseEvents(client, 'course-1', [{ title: 'A' }, { title: 'B' }]);
...
Deno.test(
  'delete error → ok:false, stage:delete, inserted reflects the successful insert',
  async () => {
