...
Deno.test('course fields fall back through the analysis field-name variants', () => {
  assertEquals(
    mapAnalysisToCourseUpdate(
      { course: { title: 'From Title', course_code: 'NEW1' } },
      EXISTING_COURSE
    ),
...
Deno.test(
  'professor falls back to instructor.name, then instructors[0].name, then existing',
  () => {
    assertEquals(
      mapAnalysisToCourseUpdate({ course: { instructor: { name: 'Dr. A' } } }, EXISTING_COURSE)
        .professor,
      'Dr. A'
    );
    assertEquals(
      mapAnalysisToCourseUpdate({ course: { instructors: [{ name: 'Dr. B' }] } }, EXISTING_COURSE)
        .professor,
      'Dr. B'
    );
...
Deno.test(
  'no schedule key in the analysis → schedule is null (course meeting info dropped)',
  () => {
    const result = mapAnalysisToCourseUpdate(
...
Deno.test(
  'normalizeEventTime: a range takes the normalised start, borrowing a trailing meridiem',
  () => {
...
Deno.test(
  'mapEventsToRows: an unparseable time and time_start both drop to null (event kept)',
  () => {
    const [row] = mapEventsToRows(
...
Deno.test(
  'countPdfPagesHeuristic: no markers found (e.g. a compressed-object-stream PDF) returns 0',
  () => {
